...
BACKUP_FILE="${CAL_FILE}.bak_theme_$(date +%s)"

if [ ! -f "$CAL_FILE" ]; then
  echo "ERROR: $CAL_FILE not found."
  exit 1
fi

cp "$CAL_FILE" "$BACKUP_FILE"
echo "📦 Backed up $CAL_FILE to:"
echo "  $BACKUP_FILE"
echo
...
export default function CalendarV1DemoScreen() {
  ...
  return (
    <SafeAreaView
      style={{
        flex: 1,
        backgroundColor: "#F0FAFC", // light Tiffany background
      }}
    >
      {/* Header */}
      <View
...
echo "✅ Patched calendar-v1-demo.tsx to use SafeAreaView and themed styles."
