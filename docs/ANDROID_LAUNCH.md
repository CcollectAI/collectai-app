...
| 4 · 2026-09-16 18:56 | jsswap 18:54 | API down, `--small` (720x1520 @320dpi ≈ 360dp) | 65/79 | 29 | 0 / 1 / 0 / 2 | **one-off**: Deal Agent's loading skeletons sit in a padding-only view, so at 360dp they pushed QuickNavBar below the fold (now a filling ScrollView). **tool**: the header band was 19% of screen HEIGHT — above the title on a short screen (7 false NO_TITLE), now measured in DP; and `subscription` ran 985 s because a hung `adb` ignored SIGTERM (now SIGKILL; 25 s). The two truncation risks the playbook had never measured — the Watchlist cluster and the barcode placeholder — are clean at 360dp |
