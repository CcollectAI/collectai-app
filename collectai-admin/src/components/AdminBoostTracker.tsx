...
export function AdminBoostTracker() {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h2 className="text-lg font-bold text-[#0D1B2A]">Boost / Spark Ads Tracker</h2>
        <AdminDemoBanner />
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div className="rounded-xl bg-white p-6 shadow-sm text-center">
          <p className="text-xs font-semibold uppercase tracking-wider text-gray-400">Organic ROI</p>
          <p className={`mt-2 text-4xl font-extrabold tabular-nums ${roiColor(organicROI)}`}>
            {organicROI >= 0 ? "+" : ""}{(organicROI * 100).toFixed(0)}%
          </p>
          <div className="mx-auto mt-3 h-2 w-32 overflow-hidden rounded-full bg-gray-100">
            <div
              className="h-full rounded-full bg-[#FF6B6B]"
              style={{ width: `${Math.min(Math.abs(organicROI) * 100, 100)}%` }}
            />
          </div>
          <p className="mt-2 text-[10px] text-gray-400">Revenue vs production cost (no ad spend)</p>
        </div>
        <div className="rounded-xl bg-white p-6 shadow-sm text-center">
          <p className="text-xs font-semibold uppercase tracking-wider text-gray-400">Boosted ROI</p>
          ...
          <p className="mt-2 text-[10px] text-gray-400">Revenue vs (production + ad spend)</p>
        </div>
      </div>

      {/* Boosted videos table */}
      <div>
        <h3 className="mb-3 text-sm font-bold uppercase tracking-wider text-gray-500">Boosted Videos</h3>
        <div className="overflow-x-auto rounded-xl border border-gray-200">
          <table className="w-full text-left text-xs">
            ...
            <tbody>
              {boostedVideos.map((v) => (
                <tr key={v.videoId} className="border-t border-gray-100">
                  <td className="max-w-[200px] truncate px-3 py-2.5 font-medium text-[#0D1B2A]">{v.hookText}</td>
      ...
      <div>
        ...
        <div className="space-y-3">
          {boostedVideos.map((v) => {
            ...
            return (
              <div key={v.videoId} className="rounded-xl bg-white p-4 shadow-sm">
                <p className="mb-2 max-w-md truncate text-xs font-medium text-[#0D1B2A]">{v.hookText}</p>
                {/* Organic bar */}
                <div className="flex items-center gap-2">
                  <span className="w-16 text-right text-[10px] font-semibold text-gray-400">Organic</span>
                ...
                <div className="mt-1 flex items-center gap-2">
                  <span className="w-16 text-right text-[10px] font-semibold text-gray-400">Boosted</span>
                  <div className="flex-1">
                    <div className="h-5 overflow-hidden rounded-full bg-gray-100">
                      <div
