...
class TCGPlayerCaller:
    ...
    async def sold_comps(
        ...
        """Get sold comparables by reusing search results with is_sold=True.

        TCGPlayer's catalog search already fetches market prices via
        ``_fetch_prices``, so we simply reuse those normalised hits
        instead of making a redundant second HTTP call to the pricing
        endpoint.
