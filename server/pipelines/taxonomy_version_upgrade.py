...
# --- SQL injection guard: only allow known identifiers in dynamic queries ---
ALLOWED_TABLES = frozenset(t["table"] for t in MIGRATION_TARGETS)
ALLOWED_COLUMNS = frozenset(
    col
    for t in MIGRATION_TARGETS
    for col in (t["category_column"], t["version_column"])
)
_IDENT_RE = re.compile(r"^[a-z_][a-z0-9_]*$")


def _validate_identifier(name: str, kind: str, allowed: frozenset[str]) -> str:
    """Validate that *name* is a known identifier.  Raises ValueError otherwise."""
    if name not in allowed:
        raise ValueError(
            f"Invalid {kind}: {name!r}. Allowed: {sorted(allowed)}"
        )
...
async def count_affected_rows(
    ...
    _validate_identifier(category_column, "column", ALLOWED_COLUMNS)
    _validate_identifier(version_column, "column", ALLOWED_COLUMNS)
    base_query = (
    ...
    try:
        ...
        for target in MIGRATION_TARGETS:
            ...
            if not dry_run:
                ...
                _validate_identifier(table, "table", ALLOWED_TABLES)
                _validate_identifier(ver_col, "column", ALLOWED_COLUMNS)
