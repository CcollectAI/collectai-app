...
Usage:
    python -m pipelines.firecrawl_events [options]
...
logger = setup_logging("collectai.firecrawl_events")
...
# Common patterns for event-like content in markdown
...
def _extract_events_from_markdown(
    ...
    # Extract all dates found in the page
    all_dates = _extract_dates(markdown)
    if not all_dates:
        return events

    # Find section headers as potential event titles
    titles = _EVENT_TITLE_RE.findall(markdown)

    # If no headers found, try to use the first sentence-like text near dates
    if not titles:
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="firecrawl_events",
        description="Crawl collectible publisher pages for event data.",
    )
...
def main() -> None:
    ...
    logger.info("Starting Firecrawl event crawler (since=%d days)", args.since)
