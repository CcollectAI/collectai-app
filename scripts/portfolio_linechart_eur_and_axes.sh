...
cp "$FILE" "$FILE.bak_axes_$TS"
echo "✅ Backup: $FILE.bak_axes_$TS"
...
export const PortfolioLineChart: React.FC<PortfolioLineChartProps> = ({
  ...
  const { path, min, max } = useMemo(() => {
    ...
    const h = 120; // taller so it feels like a real chart
    ...
    return (
      <View style={styles.emptyContainer}>
        <Text style={styles.emptyText}>
          No history yet. Add a few items to see your portfolio curve.
        </Text>
      </View>
    );
  }
  ...
  return (
    ...
    >
      ...
      <View style={styles.axisOverlay} pointerEvents="none">
        <Text style={styles.axisLabelTop}>{formatCurrencyEUR(max)}</Text>
        <Text style={styles.axisLabelBottom}>{formatCurrencyEUR(min)}</Text>
      </View>
      ...
      {width > 0 && (
        <View style={styles.svgWrap}>
          <Svg height={h} width={width}>
            ...
            {path ? (
              <Path d={path} fill="none" stroke={accentColor} strokeWidth={2.5} />
            ) : null}
          ...
          <View style={styles.xAxisRow} pointerEvents="none">
            <Text style={styles.xAxisLabel}>{startLabel}</Text>
            <Text style={styles.xAxisLabel}>{endLabel}</Text>
          </View>
        </View>
      )}
    </View>
...
echo "✅ PortfolioLineChart: EUR restored + axes cues added."
