...
if [ ! -f "$TARGET" ]; then
  echo "Items screen not found at $TARGET. Aborting."
  exit 1
fi

echo "Backing up $TARGET"
cp "$TARGET" "$TARGET.bak.items_download.$(date +%s)"
...
export default function ItemsScreen() {
  ...
  const handleDownload = async function () {
    ...
    try {
      ...
      setDownloadStatus("Done (status=" + res.status + ", ok=" + String(res.ok) + ")");
      setDownloadBody(text);
    } catch (e: any) {
      setDownloadStatus("Error while fetching export");
      setDownloadBody(String(e && e.message ? e.message : e));
    }
  };
    ...
    >
      ...
      >
        ...
        <Text style={{ color: "#4a647a", marginBottom: 8 }}>
          ...
          returns stub CSV text you can preview.
        </Text>
        <Button title="Download overview (stub)" onPress={handleDownload} />
      ...
      <Text style={{ marginTop: 16, color: "#7a8b9a", fontSize: 12 }}>
        Demo mode: categories, watchlist and export are based on mock data and a stub backend endpoint.
      </Text>
    </ScrollView>
  );
}
...
echo "Items tab updated with wired Download overview button."
