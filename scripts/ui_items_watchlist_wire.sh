...
cp "$TARGET" "$TARGET.bak.watchlist_wire_full.$(date +%s)" || true
...
export default function ItemsScreen() {
  ...
  const loadWatchlist = async function () {
    ...
    try {
      ...
      setWatchlistError("Error loading watchlist: " + msg);
      setWatchlistItems([]);
    } finally {
  ...
  const handleAddDemoToWatchlist = async function () {
    setWatchlistActionStatus("Adding demo item...");
    try {
      const url = API_BASE + "/watchlist/mine";
      ...
      const text = await res.text();
      const okText = "Done (status=" + String(res.status) + ", ok=" + String(res.ok) + ")";
      setWatchlistActionStatus(okText + " • " + text);

      // After adding, reload watchlist
      await loadWatchlist();
    } catch (e: any) {
      const msg = e && e.message ? e.message : String(e);
      setWatchlistActionStatus("Error adding demo item: " + msg);
    }
  };
  ...
  return (
    ...
    >
      ...
      <Text style={{ marginTop: 16, color: "#7a8b9a", fontSize: 12 }}>
        Demo mode: categories are local mock data. Watchlist and export talk to the backend
        using stub endpoints, so you can test wiring and payloads.
      </Text>
    </ScrollView>
  );
}
...
echo "Items tab updated: watchlist is now wired to backend (load + add demo)."
