...
if [ ! -f "$TARGET" ]; then
  echo "Add screen not found at $TARGET, creating directory."
  mkdir -p "app/(tabs)"
fi

echo "Backing up $TARGET"
cp "$TARGET" "$TARGET.bak.add_mvp_fix.$(date +%s)" || true
      ...
      {pred && (
        ...
        >
            ...
            <>
              ...
              {typeof attrs.rarity_score === "number" && (
                <Text style={{ color: "#4a647a" }}>
                  Rarity score: {String(Math.round(attrs.rarity_score * 100))} / 100
                </Text>
              )}
            </>
          )}
          {typeof pred.estimated_low === "number" &&
            typeof pred.estimated_high === "number" && (
          ...
          {typeof pred.confidence === "number" && (
            <Text style={{ color: "#4a647a" }}>
              Confidence: {String(Math.round(pred.confidence * 100))}%
            </Text>
          )}
        </View>
      )}
...
echo "Add tab updated to QuickScan MVP wired to backend."
