...
if [ -f "$TARGET" ]; then
  ...
  cp "$TARGET" "$TARGET.bak.items_fetch_debug.$(date +%s)"
else
...
async function fetchWatchlistDebug() {
  ...
  try {
    const res = await fetch(API_BASE + "/watchlist/mine", {
      method: "GET",
...
export default function ItemsScreen() {
  ...
  const handleLoad = async () => {
    ...
    try {
      const res = await fetchWatchlistDebug();
      const statusText =
  ...
  return (
    ...
    >
      ...
      <Button title="Load Watchlist (debug)" onPress={handleLoad} />
...
echo "Replaced Items screen with template-free fetch debug version."
