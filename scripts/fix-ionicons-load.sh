...
echo "→ Install icon + font deps"
npx expo install @expo/vector-icons expo-font >/dev/null

echo "→ Root layout: preload Ionicons font before first paint"
...
echo "→ Tabs: use Ionicons default import & safe glyph names"
  ...
  return (
    <Tabs
      screenOptions={{
        ...
        headerStyle: { backgroundColor: theme.colors.card }, // header white line on all pages
...
echo "→ Components: switch all to Ionicons default import"
# CompactSelect
...
# Marketplace (only the icon imports/names matter for this fix)
...
echo "→ (Optional) keep a tiny EmojiIcon noop so any stray import won't crash"
