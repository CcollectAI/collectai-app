...
echo "→ Update Tabs: rename Items labels, ensure header buttons"
  ...
  return (
    ...
    >
      ...
      <Tabs.Screen
        name="items"
        options={{
          // 1) Make the CollectAI label on menu bar say 'Items'
          title: 'Items',
          tabBarLabel: 'Items',
...
echo "→ Fix LineChart: measure container width, strict clip (never exceeds box), High/Low kept"
...
const fmtEUR_US = (n: number) => new Intl.NumberFormat('en-US', { style: 'currency', currency: 'EUR', maximumFractionDigits: 0 }).format(n);
...
export default function LineChart({ data, height = 180, gridLines = 4 }: Props) {
  ...
  const pad = 16;
  const wOuter = containerW ?? Math.min(winW, 700);
  const innerW = Math.max(0, wOuter - pad * 2);
  ...
  return (
    <View onLayout={onLayout} style={{ backgroundColor: theme.colors.card }}>
...
echo "→ Portfolio screen: white-box 'Items' title, restore Watchlist under items"
...
echo "→ Items screen: title reads 'Items', tighter spacing, share top-right, download button bottom-center"
...
export default function Items() {
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: theme.colors.bg }} contentContainerStyle={{ padding: theme.spacing.lg, gap: theme.spacing.lg }}>
      {/* 5) Page label now 'Items' on white bg, aligned with header share button */}
      ...
      {merged.map((g) => {
        ...
        return (
          <View key={g.category} style={{ gap: theme.spacing.xs }}>
            {/* 6) Tighter spacing: smaller padding & row height */}
            <Card style={{ gap: theme.spacing.sm, padding: theme.spacing.md }}>
              ...
              {/* Table header (no % column; % is under name) */}
            ...
            {/* 6) Total under the card (not part of the box), visually tight */}
      ...
      {/* 7) Bottom-centered Download Overview button */}
