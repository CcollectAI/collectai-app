...
if [ -z "$TARGET" ]; then
  echo "No Add screen found at app/(tabs)/add.tsx or app/add.tsx; creating app/(tabs)/add.tsx."
  mkdir -p "app/(tabs)"
  TARGET="app/(tabs)/add.tsx"
fi

echo "Patching $TARGET (backup will be created if existed)."
if [ -f "$TARGET" ]; then
  cp "$TARGET" "$TARGET.bak.quickscan.$(date +%s)"
fi
  ...
  return (
    ...
    >
      ...
      <Text style={{ marginBottom: 12 }}>
        For MVP, this simulates a QuickScan: one tap calls the backend and shows a demo prediction.
      </Text>
...
echo "Patched $TARGET with QuickScan demo Add screen."
